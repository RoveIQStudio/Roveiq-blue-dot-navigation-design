...
- **Injected `locationSource` is no longer stopped or disposed by the SDK.** Neither the controllers nor the React hooks (`useYouAreHere`, `useLocation`) stop or dispose a source you pass in — on unmount/teardown they only unsubscribe their own listeners. If you relied on unmount to halt an injected source's GPS watch, call `stop()`/`dispose()` on that source yourself (an explicit `stop()` call still stops an injected source).
